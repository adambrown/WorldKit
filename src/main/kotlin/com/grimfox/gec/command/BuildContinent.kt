...
//            writeHeightData("test-new-${String.format("%05d", test)}-heightMapBilinear", heightMapBilinear)
    ...
    fun generateRegionSplines(random: Random, regionSpec: Pair<Graph, Matrix<Byte>>, mapScale: Int): RegionSplines {
        val smoothing = (1.0f - (mapScale / 20.0f)).coerceIn(0.0f, 1.0f)
        val (graph, mask) = regionSpec
        val water = LinkedHashSet<Int>(graph.vertices.size)
        ...
        val borderSegments = findAllBorderSegments(random, graph, regions, water)
    ...
    fun generateWaterFlows(parameterSet: ParameterSet, inputGraph: Graph, inputMask: Matrix<Byte>, regionSplines: RegionSplines, flowGraphSmall: Graph, flowGraphMedium: Graph, flowGraphLarge: Graph, executor: ExecutorService): TextureId {
        return timeIt("generated water flow in") {
            generateWaterFlows(Random(parameterSet.seed), inputGraph, inputMask, regionSplines, flowGraphSmall, flowGraphMedium, flowGraphLarge, executor, 4096, parameterSet.mapScale)
        }
    }
