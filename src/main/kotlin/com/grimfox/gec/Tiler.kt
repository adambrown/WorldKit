...
import com.grimfox.gec.ui.widgets.Text
...
object Tiler {
    ...
    fun tileMasks(inputDir: File, inputName: String, outputDir: File, inputWidth: Int, tileWidth: Int) {
        ...
        inputDir.listFiles { path -> path.name.endsWith(".png") && (path.name.substringBefore('-').toIntOrNull() != null || path.name.substringBefore('-').equals("a", ignoreCase = true)) && path.name.substringAfter('-').substringBefore('-').equals(inputName, ignoreCase = true) }.sortedByDescending {
    ...
    fun tileHeightMap(inputFile: File, outputDir: File, outputName: String, tileWidth: Int, statusMessage: MutableReference<Text> = ref(text("")), canceled: ObservableReference<Boolean> = ref(false)) {
        ...
        for (tileX in 0 until tiles) {
            ...
            for (tileY in 0 until tiles) {
                ...
                statusMessage.value = text("Writing $outputFileName", TEXT_STYLE_LARGE_MESSAGE)
                if (canceled.value) break
