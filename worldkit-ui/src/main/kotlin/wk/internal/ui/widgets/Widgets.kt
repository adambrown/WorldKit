...
class FillImage(private val image: Int) : Fill {
    ...
    override fun draw(nvg: Long, block: Block, scale: Float) {
        nvgImagePattern(nvg, round(block.x * scale), round(block.y * scale), round(block.width * scale), round(block.height * scale), 0.0f, image, 1.0f, paint)
...
open class DynamicTextUtf8(override val data: ByteBuffer, override var style: TextStyle) : Text {
    ...
    protected fun widthInternal(nvg: Long, scale: Float, font: Int, size: Float): Float {
        stackPush().use { stack ->
            ...
            return ceil(ceil(bounds[2] - bounds[0].toDouble()) / scale).toFloat()
        }
    }
    ...
    override fun height(scale: Float) = ceil(ceil(style.size.value * scale.toDouble()) / scale).toFloat()
...
class ShapeMeshViewport3D(private val viewport: MeshViewport3D) : Shape {
    ...
    override fun draw(nvg: Long, block: Block, scale: Float) {
        ...
        nvgSave(nvg)
        nvgReset(nvg)
        viewport.onDrawFrame(ceil(block.x * doubleScale).toInt(), ceil(block.y * doubleScale).toInt(), ceil(block.width * doubleScale).toInt(), ceil(block.height * doubleScale).toInt(), ceil(block.root.height * doubleScale).toInt(), scale)
        nvgRestore(nvg)
    }
}
...
private class ShrinkGroupInternal(val blocks: MutableList<Block>, val horizontal: Boolean, override var isCalculated: Boolean = false): ShrinkGroup {
...
abstract class Block {
    ...
    fun handleNewDisabledMousePosition(nvg: Long, disabledX: Int, disabledY: Int, mouseX: Int, mouseY: Int) {
        if (this === root) {
            ...
            do {
                ...
                val mouseOverPair = if (inputOverride != null) {
                    prepareAndGetMouseOverBlock(disabledX, disabledY)
            ...
            root.handleNewDisabledMousePosition(nvg, disabledX, disabledY, mouseX, mouseY)
        }
    }
    ...
    protected abstract fun recalculatePositionAndSize()
    ...
    protected open fun prepareForIteration(shrinkGroups: MutableSet<ShrinkGroup>, onTicks: MutableList<Block.(Int, Int) -> Unit>?) {
    ...
    GlobalScope.launch {
        try {
            ...
            if (c is CancellationException) {
                LOG.info("User requested task cancellation.")
            } else {
